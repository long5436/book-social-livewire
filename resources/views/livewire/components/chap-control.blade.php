<div class="w-full flex justify-center gap-2 py-3">
    ...
    @if ($chap->order_by > 1)
        @foreach ($chaps->where('order_by', $chap->order_by - 1) as $item)
            {{-- {{ $item }} --}}
            <a href="{{ route('book.chap', [$bookSlug, $item->slug . '-' . $item->id]) }}"
                class="inline-block bg-primary text-white px-6 py-2 rounded-md text-base shadow-lg hover:opacity-90">
                Chương trước
            </a>
        @endforeach
    @endif
    ...
    <select wire:model="selectedValue" wire:change="selectChap" name="chap_book_control"
        class="chap_book_control rounded-md border border-gray-300 w-64 shadow-lg outline-primary">
